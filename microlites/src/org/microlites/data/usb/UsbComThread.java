...
public class UsbComThread extends DataSourceThread {
	...
	public void run() {
		...
		try{
			...
			while (!stop) {
				...
				if (request.queue(buffer, bufferDataLength) && request.equals(connection.requestWait())) {
					...
					if (bufferDataLength > 2) {
						...
						System.out.println("Recepci�n de paquete USB no finalizada, pero continuamos");
					}
					buffer.clear();
				}
			}
			...
			Log.w(TAG, "Algo ha pasado durante la transmisi�n");
			ex.printStackTrace();
		} finally {
			System.out.println("Cerrando conexi�n USB...");
			try	{
				request.cancel();
				request.close();
				System.out.println("Cerrada!");
			} catch (Exception ex){
				Log.w(TAG, "Algo ha pasado al cerrar la transmisi�n");
				ex.printStackTrace();
			}
		}
	}
